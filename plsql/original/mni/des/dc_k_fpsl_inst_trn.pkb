create or replace PACKAGE BODY dc_k_fpsl_inst_trn AS
   ...
   */ --------------------------------------------------------------------
   ...
   BEGIN
      ...
      --
--      lv_nom_fichero:= p_cod_sociedad||'_'||TO_CHAR(p_fec_hasta_proc,'YYYYMMDD')||'_MCONT_'||lv_secuencia||'.csv';
...
--      lv_nom_fichero:= p_cod_sociedad||'_'||TO_CHAR(p_fec_hasta_proc,'YYYYMMDD')||'_TBTRA_'||lv_secuencia||'.csv';
   ...
   */ -------------------------------------------------------
   ...
   BEGIN
      --
      lv_txt_num_externo := p_txt_num_externo;
      --
   ...
   */ -------------------------------------------------------
   ...
   --
   PROCEDURE p_carga_periodificacionesii (p_reg_period       IN       a1004814%ROWTYPE)
   IS
   BEGIN
      --
      Insert into A1004814
      Values p_reg_period;
      --
   END p_carga_periodificacionesii;
