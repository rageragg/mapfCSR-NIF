create or replace PROCEDURE BT_S_2000_2_TST
IS
   -- Cursor para obtener Siniestros Pagados
lv_fec_hasta_proc DATE;

--
   CURSOR c1 IS
   SELECT 'BT_'||a.cod_sociedad||to_char(c.fec_emision_spto,'yyyymm')||'_'||rownum IDN_BT
            ...
            ,NULL               TIP_BT
            ,NULL              IMP_IMPUESTO
       FROM A1004808 A, ( SELECT distinct h.num_sini, h.cod_cob ,
                   ...
                   FROM h7001200 h , mreserva mre
                   WHERE h.NUM_SINI = mre.NUM_SINI
                   AND h.NUM_EXP=mre.num_exp
                   AND h.COD_CTO_RVA= MRE.COD_CTO_RVA) B, A2000030 C
      WHERE a.cod_cia = b.cod_cia
        ...
        from a1004800 where idn_int_proc= A.idn_int_proc)
        and b.pagos!=0 ;

        /*group by a.cod_sociedad,c.fec_emision_spto , a.idn_int_proc  ,
      a.cod_sis_origen, c.fec_actu , a.txt_num_externo, b.cod_mon,
      'BT_'||a.cod_sociedad||to_char(c.fec_emision_spto,'yyyymm')||'_'||rownum;*/
   --
...
/*    IF  reg_c1.cod_mon_iso= '84'
    ...
    --  ptraza('p_trata_bt_proc','a','    reg_a1004810.imp_transaccion '||reg_a1004810.imp_transaccion);
      ...
      THEN
         -- se cambio la naturaleza segun el monto y transaccion roblet1
