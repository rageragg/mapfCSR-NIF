...
AS
   ...
   */ --------------------------------------------------
   ...
   g_limite         PLS_INTEGER         := 10000;--100;
   ...
   group by a.cod_sociedad,
      ...
      LOOP
         ...
         THEN
            ...
            -- Se cambia temporalmente ientras se forma el num_externo
            lv_cod_soc_ant := '0492'; --substr(l_tc_datos(1),4,4);
            --
             ptraza('p_proceso_fpsl_fichero','a', ' *** Leyendo variable  l_tc_datos(1) ' || l_tc_datos(1));
            --
            ptraza('p_proceso_fpsl_fichero','a', ' *** formando nombre fichero substr(l_tc_datos(1),4,4)  ' || substr(l_tc_datos(1),4,4));
            -- Crea fichero
...
-- v7.0               a.num_asegurados                       ||g_k_separador|| --v1.06
  ...
  --            (Select distinct  Num_riesgos From A2000030 Where num_poliza =b.num_poliza and num_spto= b.num_spto)   ||g_k_separador||
             -- roblet1 Homologacion de cc del core vs sap
              Case When a.txt_cto_coste = '1001'                        Then '9070'
                   When a.txt_cto_coste = '1002'                        Then '9051'
                   When a.txt_cto_coste = '1003'                        Then '9052'
                   When a.txt_cto_coste in('1004','1005','1006','1007') Then '9055'
                   When a.txt_cto_coste = '1012'                        Then '9056'
                   When a.txt_cto_coste in('1001','1008','1009','1010','1013','9999')  Then '9070'
              End                                    ||g_k_separador||
              a.cod_canal3                           ||g_k_separador||
              a.num_poliza                           ||g_k_separador||
              b.cod_cohorte                          ||g_k_separador||
              b.cod_cartera                          ||g_k_separador||
              '2' ||g_k_separador||    -- roblet1  decode(b.cod_onerosidad,'O',1,'N',2)   
             -- NVL(a.txt_uoa,'UOA_' || a.cod_sociedad || b.cod_cartera || '_' || b.cod_cohorte || b.cod_onerosidad || '_' || b.txt_met_val)||g_k_separador||
             NVL(a.txt_uoa,'UOA_' || a.cod_sociedad || b.cod_cartera || 
           (Select Distinct  
            Case When tip_rea in(0,1,2) then 'DI' 
            --When tip_rea=1 then 'CE' 
            --When tip_rea=2 then 'CE' 
            End Tip_rea 
            From A2000030 Where num_poliza =b.num_poliza) ||'N' || b.cod_cohorte || '_' ||
            b.cod_onerosidad || '_' || b.txt_met_val)  ||g_k_separador||
...
--v7.00              a.num_asegurados                       , -- v1.06
           ...
           (Select Distinct  
            Case When tip_rea in(0,1,2) then 'DI' 
            --When tip_rea=1 then 'AC' 
           -- When tip_rea=2 then 'CE' 
            End Tip_rea 
            From A2000030 Where num_poliza =b.num_poliza) ||'N'|| b.cod_cohorte || '_' ||
            b.cod_onerosidad || '_' || b.txt_met_val) ,
            a.idn_cancelacion                      ,
            a.fec_efec_cancelacion
       FROM a1004808 a, a1004809 b
     where a.cod_cia = b.cod_cia
     ...
     ORDER BY a.cod_sociedad,
      ...
      LOOP
         ...
         THEN
            --Es la primera iteracion, creamos el fichero inicial e inicializamos lv_cod_soc_ant
            -- Se cambia temporalmente ientras se forma el num_externo
            -- roblet1 se quita varible y se pone 0492
            lv_cod_soc_ant := '0492'; --substr(l_tc_datos(1),4,4);
            
             --
            ptraza('p_proceso_fpsl_fichero','a', ' *** p_genera_fpsl_contrato  l_tc_datos(1) ' || l_tc_datos(1));
            --
            ptraza('p_proceso_fpsl_fichero','a', ' *** p_genera_fpsl_contrato substr(l_tc_datos(1),4,4)  ' ||lv_cod_soc_ant);
            ...
            l_buffer_cab := NULL;
            --
               ptraza('p_proceso_fpsl_fichero','a', ' *** p_genera_fpsl_contrato TOTAL  ' || lv_ctos || ' cont' ||l_tc_datos.count );
         ...
         THEN
            ...
            LOOP
               ...
               THEN
                  ...
                  --
               ELSE
                 ptraza('p_proceso_fpsl_fichero','a', ' *** p_genera_fpsl_contrato ciclo  ' || i ||' contenido  ' || l_tc_datos(i));
                  --
                  IF l_buffer IS NOT NULL
                  THEN
                     --
            ...
            /*      IF l_length >= 200000000 or lv_cod_soc_ant <> substr(l_buffer,4,4)
                   THEN
                    ptraza('p_proceso_fpsl_fichero','a', ' *** p_genera_fpsl_contrato ENTRO _length >= 200000000 ' || lv_cod_soc_ant );
                    --
                    -- Cerramos el fichero actual
                    pp_cierre_fichero(p_fic => l_fic);
                    -- Comprobamos si ha cambiado el cod_sociedad
...
--      l_tc_datos t_c_datos;
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            --Es la primera iteracion, creamos el fichero inicial e inicializamos lv_cod_soc_ant
            lv_cod_soc_ant := '0492'; --substr(lv_num_externo,4,4);
            x:= 1;
            -- Crea fichero
...
--v7.00
             ...
             g.fec_efec_contrato                    ,
             g.fec_fin                              ,
             SUM(round(abs(g.imp_prima),2))              , --redondear a 2 decilames roblet1
             g.cod_mon_iso                          ,
             g.cod_fracc_pago                       ,
             g.cod_pre_pag                          ,
...
Having   SUM(g.imp_prima)!=0    -- roblet1
