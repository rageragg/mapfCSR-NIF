CREATE OR REPLACE PACKAGE BODY dc_k_fpsl_trn AS
    ...
    */ -------------------------------------------------------
    ...
    BEGIN
        ...
        --
        IF p_cod_fracc_pago IS NOT NULL AND p_cod_fracc_pago NOT IN ('1','2','3','4','5','6','10','12') THEN
            --
    ...
    */ -------------------------------------------------------
    ...
    PROCEDURE p_trata_paa IS
        ...
        BEGIN
            ...
            IF lc_comision%FOUND THEN
                ...
                INSERT INTO a1004813 (COD_CIA, NUM_POLIZA, NUM_SPTO, NUM_APLI, NUM_SPTO_APLI, COD_KMODALIDAD, COD_COB, IMP_COMIS_NETA_SPTO)
                     VALUES ( reg_251.cod_cia, reg_251.num_poliza, reg_251.num_spto, reg_251.num_apli, reg_251.num_spto_apli, 
                              greg_cobe.cod_kmodalidad, reg_251.cod_cob, reg_251.imp_comis_neta_spto
                            );
                --
            END IF;
            CLOSE lc_comision;
        ...
        --
        -- fraccion de pago 
        FUNCTION fi_cod_fracc_pago( p_cod_cia       IN a2000030.cod_cia%TYPE,
                                    p_num_poliza    IN a2000030.num_poliza%TYPE,
                                    p_num_spto      IN a2000030.num_spto%TYPE
                                  ) RETURN VARCHAR2 IS 
            --
            lv_cod_fracc_pago a2000030.cod_fracc_pago%TYPE;
            --
        BEGIN 
            --
            SELECT cod_fracc_pago 
              INTO lv_cod_fracc_pago
              FROM a2000030
             WHERE cod_cia      = p_cod_cia
               AND num_poliza   = p_num_poliza
               AND num_spto     = p_num_spto;
            --
            IF lv_cod_fracc_pago BETWEEN 1 AND 12 THEN 
                --
                IF lv_cod_fracc_pago = 11 THEN
                    lv_cod_fracc_pago := 10;
                END IF;
                --
            ELSE 
               lv_cod_fracc_pago := NULL;
            END IF;
            --
            RETURN lv_cod_fracc_pago;
            --
            EXCEPTION 
                WHEN NO_DATA_FOUND THEN 
                    lv_cod_fracc_pago := NULL;
                WHEN OTHERS THEN
                    lv_cod_fracc_pago := NULL;
            --         
        END fi_cod_fracc_pago;
        --
        -- datos comunes
        PROCEDURE pi_datos_comunes IS 
        BEGIN 
            --
            greg_paa := null;
            ...
            --
        END pi_datos_comunes;
        --
        -- Transaccion Primas contractuales: 1010
        PROCEDURE pi_trans_1010 IS 
            --
            l_periodo_envio     NUMBER(6);
            l_periodo_efecto    NUMBER(6);
            l_fec_efec_recibo   DATE;
            --
        BEGIN
            -- 
            greg_paa.fec_emi_tip_reg    := NULL;
            greg_paa.tip_registro_paa   := '1010';
            greg_paa.cod_fracc_pago     := fi_cod_fracc_pago(   p_cod_cia       => g_cod_cia,
                                                                p_num_poliza    => greg_paa.num_poliza,
                                                                p_num_spto      => greg_paa.num_spto
                                                            );
            --
            ...
            --
            l_periodo_envio     := extract( year from g_fec_hasta_proc )|| extract(month from g_fec_hasta_proc);
            ...
            IF l_periodo_envio = l_periodo_efecto THEN
                greg_period.fec_registro    := to_date (    extract(year from g_fec_hasta_proc) || 
                                                            lpad( extract(month from g_fec_hasta_proc),2,'0') || 
                                                            '01' ,'YYYYMMDD'
                                                       );
            ELSE
                greg_period.fec_registro    :=  to_date(    extract(year from greg_cobe.fec_efec_contrato)||
                                                            lpad( extract( month from greg_cobe.fec_efec_contrato),2,'0') ||
                                                            '01' ,'YYYYMMDD'
                                                       );
            END IF;  
            --
            ...
            pi_v_imp_comision;
            --
            dc_k_fpsl_inst.p_carga_periodificaciones( p_reg_paa     => greg_paa, 
                                                      p_reg_period  => greg_period
                                                    );
            --
        END pi_trans_1010;
        --
    ...
    */ -------------------------------------------------------
    ...
    PROCEDURE p_trata_datos_cobertura IS
		...
		BEGIN 
            ...
            CLOSE c_facultativo;   
            --
            IF NOT lv_existe THEN
                OPEN c_distribucion;
                FETCH c_distribucion INTO lv_cod_cia_rea;
                lv_existe := c_distribucion%FOUND;
                IF lv_existe THEN
                    greg_cobe.mca_reaseguro := 'S';
                ELSE
                    greg_cobe.mca_reaseguro := 'N';	
    ...
    BEGIN
        ...
        ptraza( p_file => 'p_trata_datos_cobertura',
                p_modo => 'a',
                p_buffer => 'p_trata_datos_cobertura => Fin'
            );
        --
    EXCEPTION
        WHEN OTHERS THEN
